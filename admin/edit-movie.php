...
// 1. التحقق من وجود معرف الفيلم
if(!isset($_GET['id'])) { die("معرف الفيلم غير موجود"); }
$id = intval($_GET['id']);

// 2. جلب بيانات الفيلم الحالية
$stmt = $conn->prepare("SELECT * FROM movies WHERE id = ?");
$stmt->bind_param("i", $id);
$stmt->execute();
$movie = $stmt->get_result()->fetch_assoc();

if(!$movie) { die("المحتوى غير موجود"); }

// جلب الأقسام الرئيسية للـ Select Box
...
<head>
    <meta charset="UTF-8">
    <title>تعديل: <?php echo $movie['title_ar']; ?></title>
    <link href="https://fonts.googleapis.com/css2?family=Cairo:wght@400;600;700;900&display=swap" rel="stylesheet">
    <style>
        /* سأضع نفس الـ CSS الخاص بصفحة الإضافة تماماً لضمان التطابق */
        :root {
            --main-color: #d4ff00;
            --bg-dark: #0a0a0a;
            --card-bg: #141414;
        ...
        #tmdb-search { width: 100%; padding: 15px 20px; background: var(--input-bg); border: 1px solid #333; color: #fff; border-radius: 10px; box-sizing: border-box; font-family: 'Cairo'; }
        ...
        input, select, textarea { padding: 12px 15px; background: var(--input-bg); border: 1px solid #222; color: #fff; border-radius: 8px; font-family: 'Cairo'; transition: var(--transition); }
        ...
        .backdrop-preview-box { width: 100%; height: 250px; border-radius: 15px; background: #000; border: 2px solid #222; overflow: hidden; position: relative; }
        .backdrop-preview-box img { width: 100%; height: 100%; object-fit: cover; opacity: 0.7; }
        .asset-input-group { background: rgba(255,255,255,0.03); padding: 15px; border-radius: 10px; border: 1px solid #222; }
        .asset-input-group label { display: block; font-size: 12px; color: var(--text-muted); margin-bottom: 8px; }
        
        .btn-submit { margin-top: 20px; width: 100%; background: var(--main-color); color: #000; padding: 18px; border: none; border-radius: 12px; font-weight: 900; cursor: pointer; font-size: 18px; transition: var(--transition); }
    </style>
</head>
<body>
...
<div class="main-content-inner">
    <h1>
        <svg width="24" height="24" viewBox="0 0 24 24" fill="var(--main-color)"><path d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25zM20.71 7.04c.39-.39.39-1.02 0-1.41l-2.34-2.34c-.39-.39-1.02-.39-1.41 0l-1.83 1.83 3.75 3.75 1.83-1.83z"/></svg>
        تعديل المحتوى: <?php echo $movie['title_en']; ?>
    </h1>
    
    <div class="tmdb-search-box">
        <label>تحديث البيانات تلقائياً من TMDB (اختياري)</label>
        <input type="text" id="tmdb-search" placeholder="ابحث لتحديث بيانات هذا الفيلم..." autocomplete="off">
        <div id="tmdb-results"></div>
    </div>

    <form action="update-movie.php" method="POST">
        <input type="hidden" name="movie_id" value="<?php echo $movie['id']; ?>">
        <input type="hidden" id="tmdb_id" name="tmdb_id" value="<?php echo $movie['tmdb_id']; ?>">
        ...
        <div class="hero-option">
            ...
            <label class="switch">
                <input type="checkbox" name="is_hero" value="1" <?php echo ($movie['is_hero'] == 1) ? 'checked' : ''; ?>>
        ...
        <div class="admin-card">
            ...
            <div class="form-grid">
                <div class="form-group">
                    ...
                    <input type="text" id="title_ar" name="title_ar" value="<?php echo $movie['title_ar']; ?>" required>
                </div>
                <div class="form-group">
                    <label>الاسم الأصلي (English)</label>
                    <input type="text" id="title_en" name="title_en" value="<?php echo $movie['title_en']; ?>" required>
                </div>
            </div>
            ...
            <div class="form-grid" style="grid-template-columns: 1fr 1fr 1fr;">
                <div class="form-group">
                    ...
                    <input type="text" id="release_year" name="release_year" value="<?php echo $movie['release_year']; ?>">
                </div>
                <div class="form-group">
                    <label>تقييم IMDB</label>
                    <input type="text" id="rating" name="rating" value="<?php echo $movie['rating']; ?>">
                </div>
                <div class="form-group">
            ...
            <div class="form-group">
                ...
                <input type="text" id="genres" name="genres" value="<?php echo $movie['genres']; ?>">
            </div>

            <div class="form-group">
                <label>قصة المحتوى</label>
                <textarea id="overview" name="overview" rows="4"><?php echo $movie['overview']; ?></textarea>
            </div>
        </div>
        ...
        <div class="admin-card">
            ...
            <div class="form-grid">
                <div class="form-group">
                    ...
                    <select name="content_type" id="content_type" required>
                        <option value="movie" <?php if($movie['content_type'] == 'movie') echo 'selected'; ?>>🎬 فيلم سينمائي</option>
                        <option value="series" <?php if($movie['content_type'] == 'series') echo 'selected'; ?>>📺 مسلسل تلفزيوني</option>
                        <option value="documentary" <?php if($movie['content_type'] == 'documentary') echo 'selected'; ?>>🎥 وثائقي</option>
                    </select>
                </div>
                <div class="form-group">
                    <label>القسم الرئيسي</label>
                    <select name="main_cat_id" id="main_cat_id" onchange="loadSubCats(this.value)" required>
                        <option value="">اختر القسم...</option>
                        <?php 
                        $main_cats->data_seek(0);
                        while($c = $main_cats->fetch_assoc()): ?>
                            <option value="<?php echo $c['id']; ?>" <?php if($movie['main_cat_id'] == $c['id']) echo 'selected'; ?>>
                                <?php echo $c['name']; ?>
                            </option>
                        <?php endwhile; ?>
                    </select>
            ...
            <div class="form-grid">
                <div class="form-group">
                    ...
                    <input type="text" id="trailer_url" name="trailer_url" value="<?php echo $movie['trailer_url']; ?>">
                </div>
                <div class="form-group">
                    <label>سيرفر المشاهدة (Embed / Link)</label>
                    <input type="text" name="watch_link" value="<?php echo $movie['watch_link']; ?>">
                </div>
            </div>

            <div class="form-group">
                <label>روابط التحميل (فصل بينها بـ | )</label>
                <textarea name="download_links" rows="2"><?php echo $movie['download_links']; ?></textarea>
            </div>
        </div>
        ...
        <div class="admin-card">
            ...
            <div class="visual-assets-container">
                <div class="poster-main-wrapper">
                    <img id="poster_preview" src="<?php echo $movie['poster_path']; ?>">
                </div>
                <div class="backdrop-wide-wrapper">
                    <div class="backdrop-preview-box">
                        <img id="backdrop_preview" src="<?php echo $movie['backdrop_path']; ?>">
                    </div>
                    <div class="form-grid" style="gap: 15px;">
                        <div class="asset-input-group">
                            <label>رابط البوستر (Poster URL)</label>
                            <input type="text" id="poster_path" name="poster_path" value="<?php echo $movie['poster_path']; ?>" oninput="document.getElementById('poster_preview').src = this.value">
                        </div>
                        <div class="asset-input-group">
                            <label>رابط الخلفية (Backdrop URL)</label>
                            <input type="text" id="backdrop_path" name="backdrop_path" value="<?php echo $movie['backdrop_path']; ?>" oninput="document.getElementById('backdrop_preview').src = this.value">
                        </div>
                    </div>
        ...
        <div class="admin-card">
            ...
            <div class="form-group">
                ...
                <textarea id="cast_members" name="cast_members" rows="2"><?php echo $movie['cast_members']; ?></textarea>
            </div>
        </div>

        <button type="submit" class="btn-submit">💾 حفظ التعديلات الآن</button>
    </form>
</div>
...
// نفس سكربت البحث في TMDB من صفحة الإضافة
    ...
    if(query.length > 2) {
        ...
        .then(data => {
            ...
            data.results.forEach(movie => {
                ...
                html += `<div class="tmdb-item" onclick="getMovieDetails(${movie.id})"><img src="${poster}"><span>${movie.title} (${year})</span></div>`;
            });
...
function getMovieDetails(id) {
    ...
    document.getElementById('tmdb-search').value = "⏳ جاري تحديث البيانات...";
    ...
    .then(data => {
        ...
        if(data.images && data.images.backdrops.length > 0) renderGallery(data.images.backdrops);
    });
}
...
function fillForm(data) {
    ...
    let originalPoster = data.poster_path;
...
function renderGallery(backdrops) {
    ...
    placeholder.innerHTML = `<div style="background:rgba(212, 255, 0, 0.02); padding:20px; border-radius:12px; margin-top:25px; border:1px dashed #333;"><h3 style="color:var(--main-color); font-size:14px; margin-bottom:15px;">🖼️ خلفيات متاحة من الفيلم:</h3><div id="gallery-grid" style="display:grid; grid-template-columns: repeat(auto-fill, minmax(120px, 1fr)); gap:12px;"></div></div>`;
    ...
    backdrops.slice(0, 10).forEach(img => {
        ...
        imgTag.src = thumb;
        imgTag.style = "width:100%; height:75px; object-fit:cover; border-radius:8px; cursor:pointer; border:2px solid transparent;";
...
// دالة تحميل الأقسام الفرعية مع اختيار القسم المخزن مسبقاً
function loadSubCats(val, selectedSubId = null) {
    ...
    .then(data => {
        ...
        data.forEach(sub => {
            let selected = (sub.id == selectedSubId) ? 'selected' : '';
...
// عند تحميل الصفحة، جلب الأقسام الفرعية للقسم الرئيسي المختار للفيلم
window.onload = function() {
    let initialMainCat = document.getElementById('main_cat_id').value;
    let initialSubCat = "<?php echo $movie['sub_cat_id']; ?>";
    if(initialMainCat) {
        loadSubCats(initialMainCat, initialSubCat);
    }
};
</script>
